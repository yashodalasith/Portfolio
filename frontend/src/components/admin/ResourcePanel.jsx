...
function toFormState(item, fields) {
  ...
  for (const f of fields) {
    ...
    else if (f.type === "lines") state[f.key] = (item?.[f.key] || []).join("\n");
...
function toPayload(formState, fields) {
  const payload = {};
  for (const f of fields) {
    if (f.type === "tags") payload[f.key] = formState[f.key].split(",").map((s) => s.trim()).filter(Boolean);
    else if (f.type === "lines") payload[f.key] = formState[f.key].split("\n").map((s) => s.trim()).filter(Boolean);
    else if (f.type === "number") payload[f.key] = Number(formState[f.key]) || 0;
...
export default function ResourcePanel({ resource, title, fields, listEndpoint, titleKey = "title" }) {
  ...
  return (
    <div>
      ...
      {editingId && form && (
        <form onSubmit={handleSave} className="mt-6 space-y-4 rounded-lg border border-white/10 bg-ink-raised p-5">
          {fields.map((f) => {
            if (f.type === "image") {
              return (
                <ImageUploadField
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving} className="rounded-full bg-cyan px-5 py-2 text-xs font-medium text-ink">
              {saving ? "Saving…" : "Save"}
            </button>
            <button
              type="button"
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex gap-3">
              <button onClick={() => startEdit(item)} aria-label="Edit" className="text-slate hover:text-cyan">
                <Pencil size={16} />
              </button>
              <button onClick={() => handleDelete(item._id)} aria-label="Delete" className="text-slate hover:text-red-400">
                <Trash2 size={16} />
              </button>
            </div>
          </li>
        ))}
        {!items.length && <p className="text-sm text-slate">Nothing here yet.</p>}
      </ul>
    </div>
  );
}
